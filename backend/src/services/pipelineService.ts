...
import { PrismaClient, Lead, Interaction, Message } from '@prisma/client'
...
export class PipelineService {
   ...
   */
  async advanceLead(leadId: string, targetStage: string, reason?: string): Promise<Lead> {
   ...
   */
  async checkAutoAdvancement(leadId: string): Promise<boolean> {
   ...
   */
  async getPipelineAnalytics(): Promise<object> {
    const startTime = Date.now()
   ...
   */
  private validateStageTransition(currentStage: string, targetStage: string): boolean {
   ...
   */
  private evaluateAutoAdvancementRules(lead: Lead & { interactions: Interaction[], messages: Message[] }): string | null {
    ...
    if (currentStage === 'OUTREACH') {
      const recentReplies = lead.messages.filter(msg => 
    ...
    if (currentStage === 'REPLIED') {
      const meetingInteractions = lead.interactions.filter(interaction =>
    ...
    if (currentStage === 'MEETING_SCHEDULED') {
      const positiveInteractions = lead.interactions.filter(interaction =>
        interaction.type === 'meeting_outcome' &&
   ...
   */
  private async triggerStageAutomation(leadId: string, stage: string): Promise<void> {
   ...
   */
  private async getStageProgressionData(): Promise<object> {
    try {
      ...
      const progressionData = stageTransitions.reduce((acc, transition) => {
        const payload = transition.payload as any
