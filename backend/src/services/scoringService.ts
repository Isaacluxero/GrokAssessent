...
import { PrismaClient, ScoringProfile, Lead } from '@prisma/client'
...
export class ScoringService {
   ...
   */
  async createScoringProfile(data: CreateScoringProfileRequest): Promise<ScoringProfileResponse> {
   ...
   */
  async getScoringProfiles(): Promise<ScoringProfileResponse[]> {
    ...
    try {
      ...
      if (!request.forceRescore && lead.score > 0) {
        ...
        return {
          leadId: lead.id,
          score: lead.score,
          factors: lead.scoreBreakdown as ScoringFactors || {
            industryFit: 0,
            sizeFit: 0,
            titleFit: 0,
            techSignals: 0
      ...
      const weightedScore = this.applyScoringWeights(aiScore.factors, profile.weights as any)
      
      // Validate against rules
      const ruleValidation = this.validateAgainstRules(lead, profile.rules as any)
   ...
   */
  private async generateAIScore(
    lead: Lead & { company: any }, 
    profile: ScoringProfile
  ): Promise<{score: number, factors: ScoringFactors, rationale: string}> {
    const startTime = Date.now()
    ...
    try {
      ...
      const prompt = `Score this lead for SDR automation software fit (0-100).

Lead: ${leadData.fullName}, ${leadData.title} at ${leadData.company?.name} (${leadData.company?.industry}, ${leadData.company?.size} employees)
...
Return JSON only:
{
   ...
   */
  private applyScoringWeights(factors: ScoringFactors, weights: any): number {
    const weightedScore = 
   ...
   */
  private validateAgainstRules(lead: Lead & { company: any }, rules: any): {multiplier: number, confidence: number} {
    if (!rules) {
   ...
   */
  private evaluateRule(lead: Lead & { company: any }, rule: string): boolean {
    try {
   ...
   */
  private mapProfileToResponse(profile: ScoringProfile): ScoringProfileResponse {
    return {
      id: profile.id,
      name: profile.name,
      weights: profile.weights as any,
