    ...
    try {
        ...
        if (query.filters) {
            try {
                ...
                logger.warn('Failed to parse filters JSON', { filters: query.filters, error: error.message });
            }
        }
 ...
 */
async function searchLeads(query, page, limit, filters, sortBy, sortOrder) {
    const where = {
        OR: [
 ...
 */
async function searchCompanies(query, page, limit, filters, sortBy, sortOrder) {
    const where = {
        OR: [
 ...
 */
async function searchInteractions(query, page, limit, filters, sortBy, sortOrder) {
    const where = {
        OR: [
 ...
 */
async function searchAll(query, page, limit, filters, sortBy, sortOrder) {
...
function calculateRelevance(item, query) {
    ...
    if (item.fullName && item.fullName.toLowerCase() === queryLower)
        score += 100;
    ...
    words.forEach(word => {
        if (item.fullName && item.fullName.toLowerCase().includes(word))
            score += 10;
